...
// NEW (2026-09-10, task #73 -- ops-side surfacing): trail check-in alerts
// already flow into ops_alerts today (lib/trail-checkin-incident-
// service.js's own upsertOpsAlert writes alert_type='trail_checkin_missing'
// on every guest report), but listOpsAlertsExpanded below previously had no
// idea what to do with that alert_type and silently mislabeled it as a
// payment_hold_failure row -- see this file's own header note added below.
const { getOpenIncidentSummaries } = require('./trail-checkin-incident-service');
...
async function listOpsAlertsExpanded({ nowIso } = {}) {
  ...
  // Phase 2.5 Trail Day (2026-09-04): gear-issue reports also live in
  // ops_alerts (see lib/gear-issue-service.js's own header for why this
  // reuses the existing generic table rather than a new one), so this
  // feed's "existing" family is no longer ALWAYS payment_hold_failure --
  // branch by alert_type instead. Gear-issue rows also need the guest's
  // name/email for the Resolve panel's quote block, which listOpsAlerts()
  // doesn't carry (a payment/hold-failure alert has no guest-authored
  // message to attribute) -- fetched here in one extra query, only when
  // there's at least one gear-issue row to enrich.
  const gearIssueBookingIds = alerts
    .filter((a) => a.alertType === 'gear_issue')
    .map((a) => a.bookingId)
    .filter(Boolean);
  let gearIssueContactByBooking = {};
  if (gearIssueBookingIds.length) {
    const contactRows = await sql`
      SELECT booking_id, contact_name, contact_email
      FROM experience_bookings
      WHERE booking_id = ANY(${gearIssueBookingIds})
    `;
    contactRows.forEach((r) => { gearIssueContactByBooking[String(r.booking_id)] = r; });
  }

  // NEW (2026-09-10, task #73): same batching shape as gearIssueContactByBooking
  // just above -- one extra query up front instead of one per alert row.
  const trailCheckinBookingIds = alerts
    .filter((a) => a.alertType === 'trail_checkin_missing')
    .map((a) => a.bookingId)
    .filter(Boolean);
  const trailCheckinSummaryByBooking = trailCheckinBookingIds.length
    ? await getOpenIncidentSummaries(trailCheckinBookingIds)
    : {};

  const existing = alerts.map((a) => {
    // NEW (2026-09-10, task #73): was previously falling through to the
    // generic branch below and rendering as "Trail checkin missing" under
    // Payment & Hold -- the real category/trail/affected-names detail that
    // upsertOpsAlert() already writes was sitting unused. See
    // lib/trail-checkin-incident-service.js's getOpenIncidentSummaries.
    if (a.alertType === 'trail_checkin_missing') {
      const summary = trailCheckinSummaryByBooking[String(a.bookingId)] || {};
      return {
        source: 'trail_checkin_incident',
        alertId: a.alertId,
        bookingId: a.bookingId,
        alertType: a.alertType,
        description: summary.categoryLabel || 'Trail check-in incident',
        urgencyTier: (a.status === 'Open' && a.urgency === 'same_day_2hr') ? 'critical' : 'urgent',
        urgency: a.urgency,
        amount: '',
        ...
        trailName: summary.trailName || '',
        affectedNames: summary.affectedNames || [],
        notes: a.notes,
      };
    }
    if (a.alertType === 'gear_issue') {
      const contact = gearIssueContactByBooking[String(a.bookingId)] || {};
      // Phase 3 Post-Adventure (2026-09-05): gearIssueService.logIntake
      // now also backs the "Left something behind? Tell us" post-return
      // escape hatch, stamping a '[GEAR RETURN NOTE] ' prefix on the
      // stored message for that case (contextLabel param, see that
      // file's own header) -- this table has no separate alert_type for
      // it (still 'gear_issue', same row shape), so branch on the
      // prefix here rather than describing every row as "reported on
      // trail day" regardless of when it actually happened. Stripped
      // back off before it reaches guestMessage so the Resolve panel's
      // quote block shows the guest's own words, not the internal label.
      const rawMessage = a.stripeErrorDetail || '';
      const returnNotePrefix = '[GEAR RETURN NOTE] ';
      const isReturnNote = rawMessage.indexOf(returnNotePrefix) === 0;
      return {
        source: 'gear_issue',
        alertId: a.alertId,
        bookingId: a.bookingId,
        alertType: a.alertType,
        description: isReturnNote ? 'Note left after gear return' : 'Gear issue, reported on trail day',
        urgencyTier: 'urgent', // never critical -- Airey's resolved decision (Urgent tier, not the Critical banner)
        urgency: a.urgency,
        amount: a.amount,
        createdAt: a.createdAt,
        status: a.status,
        resolvedAt: a.resolvedAt,
        resolvedBy: a.resolvedBy,
        guestMessage: isReturnNote ? rawMessage.slice(returnNotePrefix.length) : rawMessage, // see lib/gear-issue-service.js's header on this column reuse
        guestName: contact.contact_name || '',
        guestEmail: contact.contact_email || '',
        notes: a.notes,
      };
    }
    let tier = 'urgent';
