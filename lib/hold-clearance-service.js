...
async function listBookingsForTripDate({ tripDate }) {
  // NEW (Sept 2026 SMS build): contact_phone/sms_consent added so
  // api/trigger-deposit-holds.js can send the 2-hour hold-failed SMS
  // alongside the existing email, gated on this same consent boolean
  // every other SMS touchpoint in this repo checks.
  const rows = await query(
    `SELECT booking_id, deposit_status, contact_email, contact_name,
            contact_phone, sms_consent,
