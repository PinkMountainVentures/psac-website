...
// QR label build (Section 6): the one place that needs a single unit by
// ID outside of a write path — every write function above already does
// its own targeted UPDATE ... RETURNING and has no reason to call this.
async function getUnit({ unitId }) {
  const rows = await query(`SELECT * FROM gear_units WHERE unit_id = $1`, [unitId]);
  if (!rows.length) return null;
  return unitRowToWire(rows[0]);
}
...
async function confirmCheckoutScan({ bookingId, unitId }) {
  // QR label build (2026-09-10): `unitId` here is whatever the checkout
  // screen extracted from the scan/manual-entry value (see ops-gear-
  // checkout.html's confirmScan()) — a URL-shaped scan decodes to the
  // unit's qr_token, while manual entry is always the human-readable
  // unit_id printed on the label. Matching either column in one query
  // means both paths resolve here identically, with no separate
  // "legacy"/"manual" branch to keep in sync, and no ambiguity risk:
  // unit_id (e.g. "PL-0001") and qr_token (an 8-char hex genId()) can
  // never collide in format.
  const unitRows = await query(`SELECT * FROM gear_units WHERE unit_id = $1 OR qr_token = $1`, [unitId]);
  if (!unitRows.length) {
    return { ok: false, mismatch: { reason: 'unit_not_found', detail: 'No unit with this ID exists in inventory.' } };
  }
  const unit = unitRows[0];
  // Everything below keeps using the real human-readable unit_id (never
  // the qr_token that may have been passed in) for the check-log lookup,
  // the status UPDATE, and the returned unitId — a qr_token is a lookup
  // key here, never a value written back or displayed to staff.
  unitId = unit.unit_id;
...
module.exports = {
  ...
  findOpenAlert,
  // Units CRUD
  listUnits, getUnit, addUnit, retireUnit, markClean, markDeepCleaned,
  // Availability
