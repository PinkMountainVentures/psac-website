...
const { getWeatherSnapshot } = require('./weather-service');
...
/**
 * Resolves a signerToken to what Surface B's trail check-in/incident
 * actions need (api/waiver.js's confirmTrailReturnRoster and
 * reportTrailCheckinIncident, added 2026-09-08 alongside the full
 * roster return + SAR experience build): the bookingId (to hand to
 * lib/trail-checkin-incident-service.js, the one shared engine Surface A
 * also calls into), this signer's own participantId, and a
 * reportedByRole distinguishing the three Surface B framings Airey
 * called out for this feature -- a plain attending participant, a
 * participant who's also a certified guardian for a minor, and a
 * non-attending guardian_only signer. Reuses the same role_on_booking
 * lookup getSignerContext already does below (isGuardianOnly), plus
 * checks signer.is_guardian (Model 1's own self-declare flag, set by
 * saveWaiverSignature) to tell an attending guardian apart from a plain
 * attendee.
 */
async function resolveSignerForCheckin(signerToken) {
  const sigRows = await sql`SELECT booking_id, participant_id, is_guardian FROM waiver_signatures WHERE signer_token = ${signerToken}`;
  if (!sigRows.length) return { notFound: true };
  const signer = sigRows[0];

  const ownRows = signer.participant_id
    ? await sql`SELECT role_on_booking FROM booking_participants WHERE participant_id = ${signer.participant_id}`
    : [];
  const isGuardianOnly = !!(ownRows[0] && ownRows[0].role_on_booking === 'guardian_only');

  let reportedByRole;
  if (isGuardianOnly) {
    reportedByRole = 'guardian_only';
  } else if (signer.is_guardian) {
    reportedByRole = 'participant_guardian';
  } else {
    reportedByRole = 'participant';
  }
...
/**
 * Surface B trail-day arc (2026-09-08) -- a lighter resolver than
 * resolveSignerForCheckin above for actions that are booking-level, not
 * an incident report: Heading Out and Get Guide don't need
 * reportedByRole (there's no "who reported this" question for either),
 * just the bookingId to hand to lib/adventure-prep-service.js's
 * confirmHeadingOutByBookingId/markGuideOpenedByBookingId.
 */
async function resolveSignerBookingId(signerToken) {
  const sigRows = await sql`SELECT booking_id FROM waiver_signatures WHERE signer_token = ${signerToken}`;
  if (!sigRows.length) return { notFound: true };
  return { bookingId: sigRows[0].booking_id };
}
...
async function getSignerContext(signerToken) {
  ...
  const [apRows, rosterRows, groupSigRows, ownRows, waiverContent, feedbackRows] = await Promise.all([
    ...
    sql`
      SELECT ap.selected_trail_id, ap.assignment_method,
             ...
             t.trail_name, t.opening_description, t.photo_references, t.photo_hero_url, t.park,
             t.trailhead_name, t.trail_day_tip, t.est_time_easy_pace, t.nearest_town, t.ridewithgps_link,
             ap.heading_out_at, ap.expected_return_at, ap.trail_day_roster_json,
             ap.guide_first_opened_at, ap.trail_checkin_at, ap.trail_return_roster_json,
             ap.guest_revised_return_at
    ...
    `,
    sql`
      SELECT participant_id, display_name, age_bucket, guardian_person_id, guardian_verified_at, role_on_booking
      FROM booking_participants
      WHERE experience_booking_id = ${signer.booking_id} AND is_participating = true
      ORDER BY roster_index
    `,
    // NEW (Surface B new-tiles build, 2026-09-14): every waiver_signatures
    // row for this booking (role + status + participant_id), so
    // groupWaiverStatus below can compute "X of Y in your group have
    // signed" the same way recomputeAllWaiversComplete() already defines
    // "required signers" for this booking (owner by role, everyone else
    // by participant_id) -- see claude/psac-surface-b-new-tiles-copy-
    // proposal-2026-09-14.md section 2. Cheap, booking-scoped, safe to
    // run unconditionally alongside the other Promise.all entries.
    sql`SELECT participant_id, role, status FROM waiver_signatures WHERE booking_id = ${signer.booking_id}`,
    ...
    // Post-Adventure Check-in (2026-09-08): has THIS signer already
    // submitted their own Check-in feedback? Every Surface B signer
    // (participant, participant_guardian, guardian_only) has their own
    // booking_participants row/participant_id, unlike the booker -- see
    // lib/feedback-service.js's own header comment. waiver-signer-
    // form.js's Turn/Check-in/Closing/Steady-State sequencing (Post-
    // Adventure Phase 3 spec, section 3) reads this boolean rather than
    // querying the feedback table itself.
    signer.participant_id
      ? sql`SELECT feedback_id FROM feedback WHERE booking_id = ${signer.booking_id} AND participant_id = ${signer.participant_id} LIMIT 1`
      : Promise.resolve([]),
  ]);
  const ap = apRows[0] || null;
  const isGuardianOnly = !!(ownRows[0] && ownRows[0].role_on_booking === 'guardian_only');

  // Weather glance card (T-3+ hub refresh, Weather API integration
  // 2026-09-04) -- same weatherSnapshot shape and soft-fail posture as
  // Surface A's getContextByToken() in lib/adventure-prep-service.js.
  // Deliberately computed for every signer, not just the attending
  // main signer: the non-attending guardian's own hub renders this
  // card too (2026-09-03 guardian-hub extension), and gating the fetch
  // on isGuardianOnly here would just mean duplicating this same call
  // in two branches instead of one.
  const weatherSnapshot = await getWeatherSnapshot(ap ? ap.nearest_town : null, booking.date);
  ...
  // NEW (Surface B trail-day arc, 2026-09-08): a unified attending roster
  // (minors and adults together, everyone is_participating = true) --
  // minors/attendingAdults above stay split for the guardian-only hub's
  // own tiles, but the Heading Out sheet and the Everyone Back sheet's
  // full-roster fallback both need one combined list, same shape Surface
  // A's own roster already has.
  const attendingRoster = rosterRows.map((p) => ({ participantId: p.participant_id, name: p.display_name }));

  // NEW (Surface B new-tiles build, 2026-09-14): group waiver status for
  // the new "X of Y in your group have signed" line (copy-proposal doc
  // section 2). rosterRows is already every participating, non-
  // guardian_only person on this booking (is_participating = true,
  // see the query above) -- the exact same "group" waiverSigners() on
  // Surface A counts for its own "X of Y signed" hub-tile sublabel
  // (adventure-prep-form.js), so this mirrors that logic rather than
  // inventing a new definition: a minor counts as signed once their
  // guardian has certified (guardian_verified_at), an adult counts as
  // signed once their own waiver_signatures row has status 'signed'.
  // The owner's waiver_signatures row never carries a participant_id
  // (saveWaiverSignature only sets participantId for a non-owner
  // signerToken lookup, see that function's own comment), so the owner
  // is matched by role_on_booking = 'owner' against the row with
  // role = 'owner' instead -- same distinction
  // recomputeAllWaiversComplete() already relies on.
  const groupSigByParticipantId = {};
  let groupOwnerSigStatus = null;
  groupSigRows.forEach((r) => {
    if (r.role === 'owner') groupOwnerSigStatus = r.status;
    else if (r.participant_id) groupSigByParticipantId[r.participant_id] = r.status;
  });
  const groupWaiverSignedCount = rosterRows.filter((p) => {
    if (MINOR_AGE_BUCKETS.has(p.age_bucket)) return !!p.guardian_verified_at;
    if (p.role_on_booking === 'owner') return groupOwnerSigStatus === 'signed';
    return groupSigByParticipantId[p.participant_id] === 'signed';
  }).length;
  const groupWaiverStatus = { signed: groupWaiverSignedCount, total: rosterRows.length };
  ...
  return {
    ...
    // BUG FIX (Airey's direct request, 2026-09-10): rideWithGpsExperienceAccess
    // above is a known placeholder string (see db/schema.sql's own comment
    // on it), and every Get Guide window.open call site on this surface
    // used to read it directly with no real fallback -- worse than Surface
    // A's own (harmless) ".url on a string" miss, this one actually tried
    // to window.open() the raw placeholder text. ridewithgpsLink is the
    // trail's real, curated page (trails.ridewithgps_link), same source
    // the new T-1/T-0 email crons already use.
    ridewithgpsLink: ap ? ap.ridewithgps_link : null,
    attendingRoster,
    groupWaiverStatus,
    // NEW (Surface B trail-day arc, 2026-09-08) -- same seven fields
    // Surface A's own getContextByToken() already selects off
    // adventure_prep, see that function's own comment for what each
    // means. db/2026-09-04_add_trail_day_fields.sql,
    // db/2026-09-05_add_trail_checkin_field.sql, and
    // db/2026-09-08_add_trail_checkin_return_and_incidents.sql.
    headingOutAt: ap ? ap.heading_out_at : null,
    expectedReturnAt: ap ? ap.expected_return_at : null,
    trailDayRoster: (ap && ap.trail_day_roster_json) || null,
    guideFirstOpenedAt: ap ? ap.guide_first_opened_at : null,
    trailCheckinAt: ap ? ap.trail_checkin_at : null,
    trailReturnRoster: (ap && ap.trail_return_roster_json) || null,
    guestRevisedReturnAt: ap ? ap.guest_revised_return_at : null,
    feedbackSubmitted: feedbackRows.length > 0,
    waiverContent,
    signer: {
...
/**
 * NEW (Sept 2026 SMS build): signers with an incomplete waiver as of this
 * check, for the per-signer reminder api/check-adventure-prep-cadence.js
 * sends at its existing t5/midwindow and t3 stages -- the per-signer
 * equivalent of the booker's own unified stall reminder, but personalized
 * to the one thing a given signer, and only that signer, can do: their
 * own signature.
 *
 * Deliberately guardian-agnostic: doesn't resolve
 * booking_participants.guardian_person_id / child names the way
 * sendSignerLinksForBooking's invite copy does (that lookup needs this
 * signer's own resolved person_id, which usually isn't set yet pre-
 * signing). Flagged as a scope simplification in this session's handoff,
 * not an oversight -- a guardian-personalized variant can reuse that same
 * lookup later if wanted.
 *
 * Only non-owner signers with real contact info are worth reminding --
 * role = 'owner' rows (the booker's own waiver) are already covered by
 * the booker's unified stall reminder, not this per-signer one.
 */
async function getIncompleteSignersForReminder(bookingId) {
  const rows = await sql`
    SELECT signature_id, signer_token, signer_name, signer_email, signer_phone, sms_consent, status, person_id
    FROM waiver_signatures
    WHERE booking_id = ${bookingId} AND role = 'non_owner' AND status != 'signed'
  `;

  // NEW (2026-09-10 email/SMS audit follow-up): resolve the same
  // attending-guardian signal sendSignerLinksForBooking already resolves
  // for the invite email (see that function's own guardianChildNamesByPersonId
  // comment above), so this reminder can personalize for a known guardian
  // too instead of staying guardian-agnostic by default. This was flagged
  // as a deliberate scope cut when this reminder first shipped ("the
  // guardian lookup wasn't judged worth doing for this specific send"),
  // not a blocked one -- the lookup itself already exists, it just hadn't
  // been run here yet. A signer with no person_id (hasn't reached Confirm
  // Your Details, or person_id not yet resolved) simply gets zero rows
  // back below and falls through to the existing guardian-agnostic copy,
  // per the guardian-personalization rule (psac-copy-drafts.md house
  // style header): personalize whenever the lookup is resolvable, stay
  // agnostic only when it genuinely isn't yet, never by default.
  const minorRows = await sql`
    SELECT display_name, guardian_person_id
    FROM booking_participants
  ...
  return rows.map((r) => {
    const guardianForChildNames = (r.person_id && guardianChildNamesByPersonId[r.person_id]) || [];
    return {
      ...
      isAttendingGuardian: guardianForChildNames.length > 0,
      guardianForChildNames: guardianForChildNames,
    };
  });
}

/**
 * NEW (Airey's direct request, 2026-09-10): every non-owner signer who has
 * actually SIGNED (as opposed to getIncompleteSignersForReminder above,
 * which is the opposite filter, for waiver-reminder purposes). Backs the
 * new signer-facing gear-on-its-way/trail-day push messages
 * (api/send-gear-on-its-way.js, api/send-trail-day-message.js) -- those
 * are trip-logistics messages, not waiver nudges, so the right audience
 * is "who's actually confirmed to be on this trip," not "who still owes
 * a signature." A signer who hasn't signed yet isn't confirmed to attend,
 * so they don't get gear/trail-day logistics about a trip they may not be
 * on.
 */
async function getAttendingSignersForBooking(bookingId) {
  const rows = await sql`
    SELECT signature_id, signer_token, signer_name, signer_email, signer_phone, sms_consent
    FROM waiver_signatures
    WHERE booking_id = ${bookingId} AND role = 'non_owner' AND status = 'signed'
  `;
  return rows.map((r) => ({
...
module.exports = {
  ...
  resolveSignerForCheckin,
  resolveSignerBookingId,
  getIncompleteSignersForReminder,
  getAttendingSignersForBooking,
};
