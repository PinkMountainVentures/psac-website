...
async function sendSignerLinksForBooking(token) {
  ...
  eligibleRows.forEach((p) => {
    ...
    results.push({ name: p.display_name, email: p.email, signerToken, participantId: p.participant_id, isGuardianOnly: p.role_on_booking === 'guardian_only' });
  });
...
async function getSignerContext(signerToken) {
  ...
  const [apRows, rosterRows, waiverContent] = await Promise.all([
    sql`
      SELECT ap.selected_trail_id, ap.assignment_method, t.trail_name, t.opening_description
  ...
  return {
    ...
    selectedTrail: ap && ap.selected_trail_id ? { trailId: ap.selected_trail_id, trailName: ap.trail_name, overviewCopy: ap.opening_description } : null,
    waiverContent,
    signer: {
