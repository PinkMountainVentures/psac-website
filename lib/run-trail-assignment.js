...
async function runTrailAssignmentForBooking({ bookingId, operation }) {
  ...
  // BUG FIX (Airey's live-test report, 2026-09-03): a manual_override
  // candidate is only preserved across a 'refresh' if it still clears the
  // Tier A re-check above (this file's own header comment on that logic) --
  // when it doesn't, or on any other refresh where the previously-selected
  // trail simply isn't in the fresh candidateTrails, `selected_trail_id`
  // on adventure_prep was never being cleared. Nothing else in this
  // codebase clears it either (grep confirms selectTrail() and
  // trail-swap-service.js are the only two writers, both SETTING it, never
  // NULLing it) -- so the client kept "trailSelected: true" forever
  // against a candidate_trails row that no longer existed, which is why
  // the hub's trail card (and the Trail Recommendation tile's own
  // subtitle, both of which resolve the selected trail by filtering
  // candidateTrails for this id) silently went blank: distance/elevation/
  // difficulty/technical all fell back to their "no candidate found"
  // dashes, and the description fell back to the generic "A safe, solid
  // fit for your group." copy. The fix clears the dangling reference here
  // so the guest instead lands back in the correct "awaiting team trail"
  // state (renderInReview(), already built this session) rather than a
  // broken-looking card.
  const selectedTrailIdOrphaned = !!existingSelectedTrailId &&
    !result.candidateTrails.some((c) => c.trailId === existingSelectedTrailId);
  const finalSelectedTrailId = selectedTrailIdOrphaned ? null : existingSelectedTrailId;
  ...
  await transaction((txSql) => {
    ...
    if (selectedTrailIdOrphaned) {
      queries.push(txSql`
        UPDATE adventure_prep SET assigned_at = ${result.assignedAt}, assignment_method = ${result.assignmentMethod}, selected_trail_id = NULL
  ...
  return {
    ...
    swapRequestOpened,
    // NEW (see selectedTrailIdOrphaned comment above): the CURRENT,
    // possibly-just-cleared selected_trail_id, so callers can keep their
    // own in-memory copy in sync rather than hanging onto a stale id that
    // no longer resolves to anything in candidateTrails.
    selectedTrailId: finalSelectedTrailId,
  };
}
