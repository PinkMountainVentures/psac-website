/* ============================================
   ...
   UPDATED (2026-09-10 email/SMS audit follow-up): personalizes for a
   known attending guardian (ownerName + guardianForChildNames), mirroring
   the email variant's own guardian split. Falls back to the original
   guardian-agnostic body when isAttendingGuardian isn't set.
   ============================================ */

var { sendSms } = require('./send-sms');

function buildBody({ ownerName, tripDateFormatted, signerUrl, isAttendingGuardian, guardianForChildNames }) {
  var isGuardian = !!isAttendingGuardian && (guardianForChildNames || []).length > 0;
  var who = isGuardian ? guardianForChildNames.join(' and ') : 'you';
  return 'Palm Springs Adventure Club: ' + (ownerName || 'Your trip organizer') +
    '’s trail day is ' + (tripDateFormatted || 'coming up') +
    '. Without your signature we can’t include ' + who + '. Sign by 10pm tonight: ' + signerUrl;
}
 ...
 * @param {boolean} [opts.isAttendingGuardian]
 * @param {string[]} [opts.guardianForChildNames]
