...
function labelUrlFor(qrToken) {
  // Short on purpose (2026-09-10): the original /ops-gear-units.html?
  // unit=TOKEN form is 74 characters once the domain is included, which
  // pushed the QR to version 5 (37x37 modules) -- noticeably denser than
  // the ~1 inch of tape height any Brother label printer in this price
  // range actually gives it to work with, which is real risk for a QR
  // meant to be scanned by a phone camera in the field. This short form
  // (~51 characters) is exactly the same URL in substance -- vercel.json
  // has a redirect from /u/:token to the real ops-gear-units.html?
  // unit=:token page, so the browser (and every scanning phone) ends up
  // in the identical place -- it just gets there via a shorter link,
  // which drops the QR to version 4 (33x33) at the same error-correction
  // level. Confirmed with a real QRCode.toString() comparison, not
  // assumed: 37x37 -> 33x33 modules, an ~11% smaller/denser code for the
  // exact same physical label size. Nothing downstream needed to change
  // for this -- ops-gear-checkout.html's confirmScan() and ops-gear-
  // units.html's own deep-link handler both only ever see the final,
  // redirected long-form URL, never this short one.
  return `${getSiteUrl()}/u/${encodeURIComponent(qrToken)}`;
}
