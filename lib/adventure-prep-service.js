...
async function confirmHeadingOut(token, payload) {
  ...
  const apRows = await sql`
    ...
    FROM adventure_prep WHERE booking_id = ${booking.booking_id}
  `;
  const ap = apRows[0] || {};
  ...
  const rosterRows = await sql`
    ...
    FROM booking_participants
    WHERE experience_booking_id = ${booking.booking_id} AND is_participating = true
    ORDER BY roster_index
  `;
...
async function markGuideOpened(token) {
  ...
  return { ok: true, bookingId: booking.booking_id, guideFirstOpenedAt: new Date(rows[0].guide_first_opened_at).toISOString() };
}
