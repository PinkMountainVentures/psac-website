...
async function getContextByToken(token) {
  ...
  const [apRows, roster, waivers, contacts, waiverVersionRows] = await Promise.all([
  ...
  return {
    ...
    adventurePrep: mapAdventurePrepRow(apRows[0]),
 ...
 *
 * @param {string} token
 * @param {Array<{participantId: string, gearKit: boolean}>} updates
 * @returns {Promise<{ok: boolean, error?: string, bookingId?: string}>}
 */
async function setRosterGearKits(token, updates) {
  ...
  await transaction(list.map((u) => sql`
    UPDATE booking_participants SET gear_kit = ${!!u.gearKit}, updated_at = now()
