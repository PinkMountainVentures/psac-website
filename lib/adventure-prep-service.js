...
// Phase 2.5 Trail Day (2026-09-04): reuses this codebase's own existing
// conservative parse of trails.est_time_easy_pace (upper bound of the
// free-text range) rather than writing a second copy -- see
// confirmHeadingOut's own comment below for why that's the right reading
// for the SAR clock specifically.
const { parseEstTimeHours } = require('./trail-selection-engine');
...
function mapAdventurePrepRow(ap) {
  ...
  return Object.assign(mapped, {
    ...
    // NEW (Phase 2.5 Trail Day, 2026-09-04) -- see
    // db/2026-09-04_add_trail_day_fields.sql for what each of these means.
    headingOutAt: ap.heading_out_at,
    expectedReturnAt: ap.expected_return_at,
    trailDayRoster: ap.trail_day_roster_json || null,
    guideFirstOpenedAt: ap.guide_first_opened_at,
    // NEW (Web trail check-in, 2026-09-05) -- see
    // db/2026-09-05_add_trail_checkin_field.sql.
    trailCheckinAt: ap.trail_checkin_at,
    // NEW (full roster return + SAR experience, 2026-09-08) -- see
    // db/2026-09-08_add_trail_checkin_return_and_incidents.sql and
    // lib/trail-checkin-incident-service.js.
    trailReturnRoster: ap.trail_return_roster_json || null,
    guestRevisedReturnAt: ap.guest_revised_return_at,
  });
}
...
function mapRosterRowForContext(r) {
  return {
    ...
    // NEW (guardian-assignment UI, 2026-09-02): the frontend needs each
    // row's own person_id to match a minor's guardian_person_id back to
    // a specific attending adult (or a guardian_only row) so the
    // Attendees flow's new "who signs for this child" screen can
    // pre-select an existing assignment when the guest revisits it.
    // Read-only addition -- nothing writes personId back through this
    // mapper.
...
async function findBookingByToken(token) {
  const rows = await sql`
    SELECT booking_id, person_id, contact_name, contact_email, tier, date,
           ...
           delivery_status, delivery_time_slot, delivery_scheduled_at, gear_delivered_at, gear_delivered_by,
           return_status, pickup_service_type, pickup_scheduled_at, pickup_address_override, pickup_time_note,
           picked_up_at, gear_returned_at, deposit_status, reconciled_at, reconciled_amount_cents,
           gear_shortfall_cents, shortfall_charged_amount_cents
...
async function getContextByToken(token) {
  ...
  const [apRows, roster, waivers, contacts, waiverVersionRows, candidateTrailRows, nearestTownRows, gearCheckLogRows, feedbackRows] = await Promise.all([
    ...
    sql`
      SELECT participant_id, roster_index, display_name, email, age_bucket, fitness_level,
             ...
             pack_size_preference, gear_kit, person_id
    ...
    // 2026-09-04): the selected trail's nearest_town, resolved here
    // regardless of whether it also appears in candidateTrailRows above
    // (that query is scoped to candidate_trails rows, which don't carry
    // nearest_town, and a trail can be selected without a matching
    // candidate row surviving re-runs of trail assignment). LEFT JOIN so
    // a booking with no trail selected yet just yields no row, not an
    // error.
    sql`
      SELECT t.nearest_town, t.ridewithgps_link
      FROM adventure_prep ap
      LEFT JOIN trails t ON t.trail_id = ap.selected_trail_id
      WHERE ap.booking_id = ${booking.booking_id}
    `,
    // Gear-return card (Phase 3 Post-Adventure, 2026-09-05): per-item
    // condition/grace-deadline rows the SAME lib/gear-service.js check-in
    // flow already writes (gear_check_log, unit_id IS NOT NULL -- rows
    // without a unit_id yet are unallocated, nothing to report on here).
    // adventure-prep-form.js's computeGearReturnStatus() reads these to
    // name a missing/damaged item and its grace deadline rather than just
    // saying "checking in."
    sql`
      SELECT item_name, condition, checked_out_at, checked_in_at, grace_deadline, recovered_at
      FROM gear_check_log
      WHERE booking_id = ${booking.booking_id} AND unit_id IS NOT NULL
    `,
    // Post-Adventure Check-in (2026-09-08): has the booker already
    // submitted their own Check-in feedback? The booker's row is always
    // participant_id IS NULL, reported_by_role = 'booker' -- see
    // lib/feedback-service.js's own header comment. adventure-prep-
    ...
    sql`
      ...
      WHERE booking_id = ${booking.booking_id} AND participant_id IS NULL AND reported_by_role = 'booker'
      LIMIT 1
    `,
  ]);

  const weatherSnapshot = await getWeatherSnapshot(
    nearestTownRows[0] ? nearestTownRows[0].nearest_town : null,
    booking.date
  );
  ...
  return {
    ...
    experienceBooking: {
      ...
      // Gear delivery card (T-3+ hub, Airey's direct spec, 2026-09-05):
      // the raw fields lib/gear-service.js's existing delivery state
      // machine already writes (ready_for_delivery -> delivery_scheduled
      // -> delivered) -- no new columns. adventure-prep-form.js's
      // computeGearDeliveryStatus() turns these into the four guest-
      // facing states (Packing/Packed/Out for Delivery/Delivered).
      deliveryStatus: booking.delivery_status || '',
      deliveryTimeSlot: booking.delivery_time_slot || '',
      deliveryScheduledAt: booking.delivery_scheduled_at || null,
      gearDeliveredAt: booking.gear_delivered_at || null,
      gearDeliveredBy: booking.gear_delivered_by || '',
      // Gear return card (Phase 3 Post-Adventure, 2026-09-05): the raw
      // fields lib/gear-service.js's existing return/check-in/
      // reconciliation state machine already writes (return_status
      // pickup_scheduled -> picked_up -> returned -> checked_in;
      // deposit_status released/partial_capture/full_capture/
      // shortfall_charged/full_capture_pending_review) -- no new
      // columns. adventure-prep-form.js's computeGearReturnStatus() turns
      // these, plus gearCheckLog below, into the guest-facing states
      // (Return Due/On The Way/Picked Up/Checking In/Double-Checking/
      // Checked In/Wrapping Up/Charge Applied).
      returnStatus: booking.return_status || '',
      pickupServiceType: booking.pickup_service_type || '',
      pickupScheduledAt: booking.pickup_scheduled_at || null,
      pickupAddressOverride: booking.pickup_address_override || '',
      pickupTimeNote: booking.pickup_time_note || '',
      pickedUpAt: booking.picked_up_at || null,
      gearReturnedAt: booking.gear_returned_at || null,
      depositStatus: booking.deposit_status || '',
      reconciledAt: booking.reconciled_at || null,
      ...
      gearCheckLog: gearCheckLogRows.map((r) => ({
    ...
    adventurePrep: Object.assign(mapAdventurePrepRow(apRows[0]) || {}, {
      ...
      // BUG FIX (Airey's direct request, 2026-09-10): the Get Guide button
      // (openGuide() and every other window.open call site below) has
      // always fallen back to the generic https://ridewithgps.com/
      // homepage for every booking -- rideWithGpsExperienceAccess is a
      // known placeholder string (db/schema.sql's own comment says so:
      // "No real RideWithGPS integration exists yet"), so its .url read
      // was always undefined. Until a real integration exists,
      // ridewithgpsLink is the trail's real, curated page
      // (trails.ridewithgps_link), the same source the T-1/T-0 email
      // crons already use for their own Get Guide links -- see
      // lib/delivery-handoff-note.js's neighboring crons and this repo's
      // 2026-09-10 SMS-support build handoff for the full history.
      ridewithgpsLink: nearestTownRows[0] ? nearestTownRows[0].ridewithgps_link : null,
    }),
 ...
 */
/**
 * Phase 2.5 Trail Day -- backs the "Heading Out" roster-confirm sheet.
 * Writes the real start timestamp, a snapshot of who's actually present
 * today (defaults everyone attending -- absentParticipantIds only lists
 * who got unchecked), and the computed expectedReturn.
 *
 * Idempotent: a second call (a retried request, a guest tapping twice)
 * returns the ALREADY-written values instead of recomputing a new, later
 * expectedReturn off a second "now" -- the SMS STARTED path (lifecycle-
 * alerts proposal, not built by this pass) and the hub tap both need to
 * be able to fire in either order without one clobbering the other's
 * timestamp.
 *
 * Pace math -- resolved directly by Airey, trail-day design pass,
 * 2026-09-04: expectedReturn always uses the trail's easy-pace estimate
 * applied to the FULL roster, never the confirmed/toggled headcount and
 * never the group's own stated fitness pace. Deliberately the most
 * conservative reading available, so a fast or short-handed group is
 * never checked in on artificially early. absentParticipantIds affects
 * ONLY the roster snapshot (who the eventual return check-in needs to
 * hear from), never this calculation.
 */
async function confirmHeadingOut(token, payload) {
  const booking = await findBookingByToken(token);
  if (!booking) return { ok: false, error: 'Invalid or expired link' };
  return confirmHeadingOutByBookingId(booking.booking_id, payload);
}

/**
 * Surface B trail-day arc (2026-09-08) -- confirmHeadingOut's real core,
 * split out so both surfaces can call the same logic once resolved to a
 * bookingId (Surface A resolves via token above, Surface B resolves a
 * signerToken via lib/waiver-service.js's resolveSignerBookingId), same
 * split this codebase already applied to confirmTrailReturnRoster/
 * reportTrailCheckinIncident (lib/trail-checkin-incident-service.js) this
 * week. Everything below was already bookingId-based, token resolution
 * was the only thing coupling this to Surface A.
 */
async function confirmHeadingOutByBookingId(bookingId, payload) {
  const apRows = await sql`
    SELECT selected_trail_id, heading_out_at, expected_return_at, trail_day_roster_json
    FROM adventure_prep WHERE booking_id = ${bookingId}
  `;
  const ap = apRows[0] || {};

  if (ap.heading_out_at) {
    return {
      ok: true,
      bookingId,
      headingOutAt: new Date(ap.heading_out_at).toISOString(),
      expectedReturnAt: ap.expected_return_at ? new Date(ap.expected_return_at).toISOString() : null,
      roster: ap.trail_day_roster_json || [],
  ...
  const absentSet = new Set(((payload && payload.absentParticipantIds) || []).map(String));
  const rosterSnapshot = rosterRows.map((r) => ({
    participantId: r.participant_id,
    name: r.display_name,
    present: !absentSet.has(String(r.participant_id)),
  }));

  let easyPaceHours = null;
  if (ap.selected_trail_id) {
    const trailRows = await sql`SELECT est_time_easy_pace FROM trails WHERE trail_id = ${ap.selected_trail_id}`;
    if (trailRows.length) easyPaceHours = parseEstTimeHours(trailRows[0].est_time_easy_pace);
  }

  const headingOutAt = new Date();
  const expectedReturnAt = easyPaceHours != null
    ? new Date(headingOutAt.getTime() + easyPaceHours * 3600000)
    : null;

  await sql`
    UPDATE adventure_prep
    SET heading_out_at = ${headingOutAt.toISOString()},
        expected_return_at = ${expectedReturnAt ? expectedReturnAt.toISOString() : null},
        trail_day_roster_json = ${JSON.stringify(rosterSnapshot)}
  ...
  return {
    ok: true,
    bookingId,
    headingOutAt: headingOutAt.toISOString(),
    expectedReturnAt: expectedReturnAt ? expectedReturnAt.toISOString() : null,
    roster: rosterSnapshot,
  };
}

/**
 * Phase 2.5 Trail Day -- "downloaded" itself isn't observable from the
 * backend (it happens inside RideWithGPS, on the guest's own phone), so
 * this records the one real proxy signal available: has the guest ever
 * tapped Get Guide. First-tap-wins (COALESCE), never overwritten once set
 * -- a guest re-opening the guide later shouldn't reset "first opened."
 */
async function markGuideOpened(token) {
  const booking = await findBookingByToken(token);
  if (!booking) return { ok: false, error: 'Invalid or expired link' };
  return markGuideOpenedByBookingId(booking.booking_id);
}

// Surface B trail-day arc (2026-09-08) -- same bookingId-core split as
// confirmHeadingOut above.
async function markGuideOpenedByBookingId(bookingId) {
  const rows = await sql`
    UPDATE adventure_prep
    SET guide_first_opened_at = COALESCE(guide_first_opened_at, NOW())
    WHERE booking_id = ${bookingId}
    RETURNING guide_first_opened_at
  `;
  if (!rows.length) return { ok: false, error: 'Invalid or expired link' };
  return { ok: true, bookingId, guideFirstOpenedAt: new Date(rows[0].guide_first_opened_at).toISOString() };
}

/**
 * Web trail check-in (2026-09-05, superseded 2026-09-08) -- the original
 * single-tap "I'm Back" design here was replaced by a real roster
 * confirm ("Everyone back from [trail]?") plus a six-option "What's
 * going on?" triage, per Airey's direct correction: "the client told us
 * exactly who headed out. i need to know who came back." That full
 * design (six rounds of review, grounded in PSAC's own Emergency &
 * Check-In Protocol) now lives in its own file,
 * lib/trail-checkin-incident-service.js, callable from every surface --
 * see that file's own header. This file no longer has a
 * confirmTrailCheckin() of its own; findBookingByToken() below is what
 * every surface's API layer still uses to resolve Surface A's own token
 * before calling into that shared service.
 */
...
module.exports = {
  ...
  setRosterGearKits,
  confirmHeadingOut,
  confirmHeadingOutByBookingId,
  markGuideOpened,
  markGuideOpenedByBookingId,
