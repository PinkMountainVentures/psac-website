...
function runTrailSelection(input) {
  ...
  // Preserve any existing manual_override entry across a refresh (PRD
  // Section 2, Amendment 2). This engine never removes or edits it.
  let manualEntries = [];
  if (input.operation === 'refresh' && Array.isArray(input.existingCandidateTrails)) {
    manualEntries = input.existingCandidateTrails.filter((e) => e.source === 'manual_override');
  }
  ...
  const swapRequestNeeded = qualifyingCount < 3;
  ...
  return {
    ...
    swapRequestNeeded,
    swapRequestGuestConcernSummary: swapRequestNeeded
      ? `system-generated: rules engine returned only ${qualifyingCount} qualifying trail(s) for this booking`
      : null,
  };
}
