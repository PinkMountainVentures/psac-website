...
// computeGroupCeilings below used to match a roster entry's `fitness` value
// against FITNESS_ORDER with a plain `indexOf`, which only ever matches the
// exact strings 'Easygoing' / 'Comfortable' / 'Strong'. The real booking
// flow's roster step writes descriptive labels instead — confirmed live,
// e.g. "Strong / experienced" — which never equals any FITNESS_ORDER entry,
// so `indexOf` always returns -1 and every roster member is silently
// ignored, leaving `leastFitTier` stuck at its seed value ('Strong', the
// most permissive tier) no matter who's actually on the booking. That's a
// fail-OPEN bug on a Tier A safety ceiling — the same bug class
// apps-script/trail-swap-actions.gs's own trailSwap_fitnessTierKey_ already
// had to fix in its separate, hand-duplicated copy of this exact check (see
// that file's header comment); this mirrors that fix's case-insensitive,
// substring-based matching here in the real engine so both
// implementations agree on what a given roster label means.
function fitnessTierKey_(value) {
...
function computeGroupCeilings(roster, technicalComfort) {
  ...
  roster.forEach((person) => {
    const tier = fitnessTierKey_(person.fitness);
    if (
      tier !== null &&
...
/**
 * Park/date availability for ONE trail's park, independent of any other
 * Tier A check (roster, difficulty, etc.) — the exact rule
 * checkTrailSafety uses for its `park_date_availability` check, extracted
 * so callers that only care about "is this park open on this date" (the
 * booking-flow date picker; anything else that needs to reason about park
 * hours without a full booking context) can reuse it verbatim rather than
 * re-deriving the same season-range/day-of-week logic a second time.
 *
 * Fails CLOSED on a park with zero matching Park Access rows (PRD Section
 * 3: "no matching Park Access row exists ... treat the trail as not open,
 * not as open by default") — same posture as checkTrailSafety always had.
 *
 * @param {string} park - a trail's normalized `park` field (may be empty)
 * @param {Array<object>} parkAccessRows - normalized Park Access rows
 * @param {Date} referenceDate
 * @returns {boolean}
 */
function checkParkDateAvailability(park, parkAccessRows, referenceDate) {
  if (!park) return true; // no Park value = no restriction
  const matchingRows = (parkAccessRows || []).filter(
    (row) => row.park && row.park.trim().toLowerCase() === park.trim().toLowerCase()
  );
  if (matchingRows.length === 0) return false; // fail closed — see README
...
/**
 * Whether AT LEAST ONE currently-bookable trail's park is open on
 * `referenceDate` — the rule the booking-flow date picker needs (PRD/
 * build-checklist: "an open day is defined generically by whichever
 * trail's hours/days apply, not hardcoded to any one park", so this
 * unions across every bookable trail's park rather than checking a single
 * hardcoded park). A date with zero open bookable-trail parks would leave
 * Trail Selection's Tier A pool empty for every candidate — nothing to
 * assign — so gating the date picker on this is a real correctness rule,
 * not just a UX nicety.
 *
 * @param {Array<object>} trails - normalized trail rows (only `bookable`
 *   and `park` are read)
 * @param {Array<object>} parkAccessRows - normalized Park Access rows
 * @param {Date} referenceDate
 * @returns {boolean}
 */
function isAnyBookableTrailOpen(trails, parkAccessRows, referenceDate) {
  const bookableTrails = (trails || []).filter((t) => t.bookable === true);
  if (!bookableTrails.length) return true; // nothing to gate against — don't strand every date closed
  return bookableTrails.some((t) => checkParkDateAvailability(t.park, parkAccessRows, referenceDate));
}
...
function checkTrailSafety(trail, ctx, parkAccessRows, referenceDate) {
  ...
  //    trail as not open, not as open by default"). Extracted into its own
  //    exported function (2026-09-03) so the booking-flow date picker can
  //    reuse the exact same rule instead of a second, potentially-drifting
  //    copy — see checkParkDateAvailability below and its caller in
  //    lib/booking-open-days-service.js.
  checks.park_date_availability = checkParkDateAvailability(trail.park, parkAccessRows, referenceDate);
...
// FIX (2026-09-14, per Airey's request + Section 7/8 of the trail-selection
// diagnosis doc): the final tie-break used to be plain `trailId` ascending,
// which is fully deterministic PER BOOKING (good -- staff/guests can ask
// "why this trail?" and get a stable answer on re-run) but ALSO fully
// deterministic ACROSS every booking with an identical profile -- the same
// tied trail always wins, for every guest, forever. Confirmed via the
// automated test harness (scripts/test-trail-selection.js): a common
// "Strong/experienced, no preference picks" profile has 8 of 16 qualifying
// trails tied on every ranking criterion above this tie-break, and the old
// rule always resolved that 8-way tie to the same single trail.
//
// Fix: break ties with a stable hash of `bookingId + trailId` instead of
// `trailId` alone. Same booking, re-run any number of times, still resolves
// identically (bookingId never changes -- reproducibility preserved).
// Different bookings with an identical profile spread across the tied set
// instead of collapsing onto one trail (bookingId differs -- variety
// achieved). Deliberately NOT true randomness, for exactly that reason: a
// random draw would break the "same booking, same answer" guarantee this
// design leans on for auditability.
//
// A simple FNV-1a-style string hash is enough here -- doesn't need to be
// cryptographically strong, just stable and reasonably evenly distributed
// (Section 7's own proposal explicitly allows this). Falls back to the old
// `trailId`-ascending rule if no `bookingId` is available on the context
// (defensive only -- runTrailSelection always sets one from the booking).
...
function byTrailIdAscending(a, b) {
  return a.trail.trailId < b.trail.trailId ? -1 : a.trail.trailId > b.trail.trailId ? 1 : 0;
}

/**
 * Final tie-break: stable per-booking hash instead of plain `trailId`
 * ascending. See the FIX comment above this section for the reasoning.
 */
function tieBreak(a, b, ctx) {
  if (!ctx.bookingId) return byTrailIdAscending(a, b); // defensive fallback only
  const ha = stableHash(`${ctx.bookingId}:${a.trail.trailId}`);
  const hb = stableHash(`${ctx.bookingId}:${b.trail.trailId}`);
  if (ha !== hb) return ha - hb;
  return byTrailIdAscending(a, b); // extremely unlikely hash collision -- stay fully deterministic
}
...
function runTrailSelection(input) {
  ...
  const ctx = {
    ...
    // Used only by the ranking tie-break (see tieBreak() above) -- never a
    // safety/filter input, so its absence can't affect Tier A/B correctness.
  ...
  const algorithmicEntries = slots.map((slot) => ({
    ...
    // NEW (Round 2 build, Section 9 item 2): technicalRating was already
    // computed on the normalized trail row for the Tier A ceiling check
    // above, but never copied into the candidateTrails output entry, so it
    // never reached the client. distance/elevation are new normalize.js
    // fields (see that file's own flag on the real column-name guess).
    // Surfacing all four here is the entire fix for the "Choose Your
    // Trail" stat grid the Round 2 mockups/handoff doc call for — this
    // engine's own filtering/ranking logic is unchanged.
    technicalRating: slot.trail.technicalRating,
    distance: slot.trail.distance,
    elevation: slot.trail.elevation,
    source: 'rules_v1',
  }));
  ...
  if (input.operation === 'refresh' && Array.isArray(input.existingCandidateTrails)) {
    ...
    // BUG FIX (Airey's live-test report, 2026-09-03): this used to keep
    // the prior entry object as-is -- {trailId, source} and nothing else,
    // since that's genuinely all run-trail-assignment.js's
    // existingCandidateTrails carries in (candidate_trails only stores
    // rank/source/trail_id/matched_attributes/difficulty_rating/
    // technical_rating/distance/elevation, and the caller only selects
    // trail_id + source for this — see that file's own header comment,
    // point 2, on why the richer display fields aren't duplicated there).
    // The trail survived the safety re-check below and stayed in
    // candidateTrails, so the "orphaned selectedTrailId" bug fixed
    // alongside this one didn't fire and the guest kept seeing a card for
    // the right trail -- but every field this function is actually
    // responsible for producing (matchedAttributes/difficultyRating/
    // technicalRating/distance/elevation, and the display fields too)
    // came back undefined, which is what rendered as dashes and the
    // generic "A safe, solid fit for your group." fallback on the client.
    // Rebuilt from `trail` (already being looked up right here for the
    // safety check) using the exact same shape algorithmicEntries above
    // uses, so a preserved manual override is indistinguishable from a
    // freshly-computed entry except for its source.
    const survivingManualEntries = [];
    priorManualEntries.forEach((entry) => {
      const trail = (input.trails || []).find((t) => t.trailId === entry.trailId);
      if (!trail) return; // trail no longer exists in the Trail Database at all — can't re-check, don't silently keep it
      const safety = checkTrailSafety(trail, ctx, input.parkAccessRows, referenceDate);
      if (!(safety.checks.bookable && safety.checks.park_date_availability && safety.checks.seasonal_safety)) return;
      survivingManualEntries.push({
        trailId: trail.trailId,
        trailName: trail.trailName,
        overviewCopy: trail.overviewCopy,
        photoUrl: trail.photoUrl,
        rideWithGpsExperienceAccess: null,
        park: trail.park,
        trailheadLocation: trail.trailheadLocation,
        oneTripTip: trail.oneTripTip,
        matchedAttributes: matchedAttributesFor(trail, ctx.bestForAttributes),
        difficultyRating: trail.difficulty,
        technicalRating: trail.technicalRating,
        distance: trail.distance,
        elevation: trail.elevation,
        source: 'manual_override',
      });
    });
    manualEntries = survivingManualEntries;
...
module.exports = {
  ...
  runTrailSelection,
  checkTrailSafety,
  checkParkDateAvailability,
  isAnyBookableTrailOpen,
