...
function mapCandidateTrailRow(r) {
  return {
    ...
    // Golden Hour photo infrastructure (2026-09-09): the new
    // vertical/full-bleed hero slot, preferred over photoUrl above by
    // the post-adventure hero card when present -- see heroCardHtml
    // call sites in adventure-prep-form.js / waiver-signer-form.js.
    photoHeroUrl: r.photo_hero_url || null,
    park: (r.park || '').trim(),
    trailheadLocation: r.trailhead_name,
    oneTripTip: r.trail_day_tip || null,
    // NEW (non-attending guardian branch, 2026-09-03): only populated
    // when the caller's own query selects est_time_easy_pace (today just
    // waiver-service.js's getSignerContext, for Part 5's "The Day" tile's
    // expected-return estimate) -- parseEstTimeHours(undefined) is a safe
    // null for every other caller that doesn't select it.
    estTimeEasyPaceHours: parseEstTimeHours(r.est_time_easy_pace),
  };
}
