...
async function finalizePendingKitChange({ bookingId, beforeT3Cutoff }) {
  ...
  if (outcome === 'charged' || outcome === 'refunded') {
    ...
    let writeBackResult;
    try {
      writeBackResult = await callBookingsWebApp('adventurePrep_finalizeKitChange', {
        bookingId,
        newConfirmedKitCount: requested,
        oldConfirmedKitCount: currentConfirmed,
        delta,
        ...
        beforeT3Cutoff: !!beforeT3Cutoff,
        staffNotes: '',
        // BUG FIX (payment-review, Aug 2026, Medium #38): names the baseline
        // `delta` (and therefore the Stripe charge/refund amount above) was
        // actually computed from, so the write-back can detect — and
        // refuse, rather than silently clobber — a concurrent staff kit-
        // count correction that landed in the gap between this read and
        // this write. See adventurePrep_finalizeKitChange's own comment.
    ...
    // BUG FIX (payment-review, Aug 2026, Medium #38): a stale write-back
    // isn't an exception — callBookingsWebApp resolves normally with
    // {ok:false, stale:true, ...}. Without this check, that response fell
    // straight through to the normal 'charged'/'refunded' return below,
    // silently reporting success to api/process-pending-kit-changes.js even
    // though the Sheet was never actually updated (a concurrent staff kit-
    // count correction changed confirmedKitCount out from under this read,
    // and the write-back correctly refused to clobber it). The Stripe
    // action above already happened for real either way — flag it for
    // manual reconciliation instead of silently losing track of the
    // mismatch.
      ...
      try {
        await callBookingsWebApp('opsAlerts_recordAlert', {
          ...
          stripeErrorDetail: (outcome === 'charged' ? 'Charge ' : 'Refund ') + stripeTransactionId + ' for $' + refundOrChargeAmount
            ...
            + ' — something else (most likely a staff manual correction) changed it in between. The Sheet was NOT overwritten and still reflects that other change. Needs manual reconciliation: confirm the guest was correctly charged/refunded and that confirmedKitCount, the Gear Check Log, and the deposit hold all agree.',
          urgency: 'urgent_same_day',
        }, { retries: 2 });
      } catch (alertErr) {
        // eslint-disable-next-line no-console
