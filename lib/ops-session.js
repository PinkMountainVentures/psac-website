...
// MOVED HERE (payment-review, Aug 2026, Medium #47) from api/ops-auth.js,
// which used to be the only place this list was read — checked once, at
// login, to decide whether to issue a cookie at all. Nothing after that
// ever looked at it again for the life of the cookie (up to the full
// 12-hour SESSION_TTL_MS). A de-listed/offboarded staffer's existing
// cookie kept working exactly as before until it naturally expired. Now
// the single source of truth, imported by api/ops-auth.js too, so both
...
function requireStaffSession(req) {
  try {
    ...
    // BUG FIX (payment-review, Aug 2026, Medium #47): the cookie's own
    // signature/expiry being valid only proves it was legitimately issued
    // at some point in the last 12 hours — it says nothing about whether
    // that staffer is STILL allowed in right now. Re-checking the live
    // allowlist on every request (not just at login) means removing
    // someone from ALLOWED_STAFF_EMAILS takes effect on their very next
    // action instead of waiting out the rest of their session TTL.
    const email = String(payload.email).toLowerCase();
    if (allowedEmails().indexOf(email) === -1) return null;
    return { email };
  } catch (e) {
    return null;
  }
...
module.exports = { issueSessionCookie, clearSessionCookie, requireStaffSession, allowedEmails, COOKIE_NAME };
